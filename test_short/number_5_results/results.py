...
with open('results.txt', 'r') as file:
    ...
    for i in range(len(lines)):
        ...
        if line.startswith('Comparison between'):
            ...
            normalized_number = (new_count / (reactant_count / 100000)) / 100000
            row = reactant_dict[reactant.split("2")[0]]
            col = product_dict[product]
            print(f"Processing: {reactant_clean} -> {product}, row: {row}, col: {col}")
            matrix[row, col] = normalized_number

# Create row and column names
row_names = [reactant for reactant, _ in sorted(reactant_dict.items(), key=lambda x: x[1])]
