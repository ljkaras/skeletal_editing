    ...
    for batch_num, i in enumerate(range(1, len(lines), batch_size), start=1):
        ...
        for result_set, symmetric_molecules_count in batch_results:
            ...
            for name, count in symmetric_molecules_count.items():
                overall_symmetric_molecules_count[name] = (
                    overall_symmetric_molecules_count.get(name, 0) + count
                )
...
if __name__ == "__main__":
    ...
    for framework_name in frameworks:
        ...
        smarts_file_path = os.path.join(
            script_dir, "smarts_files", f"smarts_{base_filename}.csv"
        )
        ...
        output_files = {
            name: os.path.join("products", f"{base_filename}_{name}.csv")
