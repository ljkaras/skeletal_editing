...
def group_files_by_pattern(folder_path, output_file_path):
    ...
    for file_path in csv_files:
        ...
        pattern_key = filename.split('2')[-1].split('.')[0]  # Extract the pattern after "2" in the filename
    ...
    with open(output_file_path, 'w') as outfile:
        for pattern_key, file_paths in files_by_pattern.items():
            for file_path in file_paths:
                ...
                comparison_file_path = os.path.join('frameworks/', 'library_' + comparison_name + ".csv")
