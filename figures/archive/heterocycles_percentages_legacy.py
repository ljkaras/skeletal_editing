...
for i, (heterocycle, percentage) in enumerate(zip(df["Heterocycle"], df["Percentage"])):
    if mol := mols.get(heterocycle):
        ...
        imgbox = OffsetImage(
            img, zoom=0.5, resample=True, alpha=1.0
        )  # Set alpha to 1.0 for full visibility
        ab = AnnotationBbox(
            imgbox,
            ((i + 0.5) / num_cols, 0.7),  # Adjusted y-coordinate for structures
            xybox=(0, 0),
    ...
    ax.text(
        (i + 0.5) / num_cols,
        0.6,
        f"{percentage:.2f}%",  # Adjusted y-coordinate for percentages
        ha="center",
        va="center",
        fontsize=12,
        color=color,
