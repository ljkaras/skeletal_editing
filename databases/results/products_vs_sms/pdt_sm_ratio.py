...
# sets framework list
frameworks = ['pyridine', 'pyridazine', 'pyrimidine', 'pyrazine', 'pyrrole', 'pyrazole',
        'imidazole', 'thiazole', 'oxazole', 'isoxazole', 'furan']
...
for database in database_list:
    ...
    unique_count = np.loadtxt(f'unique_dfs/unique_df_{database}.csv', delimiter=',', dtype=str)
    ...
    for i in range(num_heterocycles):
        for j in range(num_heterocycles):
            if i != j:
                pdt_sm_ratios_arr[i, j] = float(unique_count[i, j]) / float(sm_count[i, j])
    ...
    # exports df to csv
    pdt_sm_ratios_df.to_csv(f'sm_pdt_ratios_dfs/sm_pdt_ratio_df_{database}.csv', index=True)
