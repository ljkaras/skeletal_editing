...
# Framework list
frameworks = [
    'pyridine', 'pyridazine', 'pyrimidine', 'pyrazine', 'pyrrole',
    'pyrazole', 'imidazole', 'thiazole', 'oxazole', 'isoxazole', 'furan'
]
...
for database in database_list:
    ...
    raw_data_df = pd.read_csv(f'../percent_rel_common_dfs/percent_rel_common_df_{database}.csv', index_col=0)
    raw_data = raw_data_df.values  # Convert to NumPy array

    # Load the normalization values, skipping the header
    normalizers_df = pd.read_csv(f'../max_common_dfs/max_common_df_{database}.csv', index_col=0)
    ...
    normalized_common_df = pd.DataFrame(normalized_common_arr, index=frameworks, columns=frameworks)

    # Export DataFrame to CSV
    normalized_common_df.to_csv(f'../normalized_common_dfs/normalized_common_df_{database}.csv', index=True)

    print(f"Normalized data for {database} has been saved to '../normalized_common_dfs/normalized_common_df_{database}.csv'.")
