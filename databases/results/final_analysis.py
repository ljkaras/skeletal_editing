...
def count_sm(frameworks, results_lines):
    ...
    sm_count_arr = np.zeros((num_frameworks, num_frameworks), dtype=int, order="C")
    ...
    sm_count_df = pd.DataFrame(sm_count_arr, index=frameworks, columns=None)
...
def count_unique(frameworks, results_lines):
    ...
    # initiate blank arrays for collecting results
    unique_count_arr = np.zeros(
        (num_frameworks, num_frameworks), dtype=float, order="C"
    )
    ...
    unique_count_df = pd.DataFrame(
        unique_count_arr, index=frameworks, columns=frameworks
    )
...
def count_common(frameworks, results_lines):
    ...
    for idx1, framework1 in enumerate(frameworks):
        for idx2, framework2 in enumerate(frameworks):
            if framework1 != framework2:
                line_prefix_match = (
                    f"Number of common molecules in {framework1}2{framework2}"
                )
    ...
    # converts array to df
    # SMs are listed on left side column, products are listed across the top
    common_count_df = pd.DataFrame(
        common_count_arr, index=frameworks, columns=frameworks
    )
...
def count_new(frameworks, results_lines):
    ...
    # initiate blank arrays for collecting results
    new_count_arr = np.zeros((num_frameworks, num_frameworks), dtype=float, order="C")
    ...
    for idx1, framework1 in enumerate(frameworks):
        for idx2, framework2 in enumerate(frameworks):
            if framework1 != framework2:
                line_prefix_match = (
                    f"Number of new molecules in {framework1}2{framework2}"
                )
    ...
    # converts array to df
    # SMs are listed on left side column, products are listed across the top
    new_count_df = pd.DataFrame(new_count_arr, index=frameworks, columns=frameworks)

    return new_count_arr, new_count_df


def GenerateHeatmap(
    dataframe, title, filename, directory, database, total_molecule_count
):
    # Define figure size
    ...
    ax.set_title(
        title, fontsize=10, fontweight="extra bold", family=fontfamily, loc="left"
    )
    ...
    props = dict(
        boxstyle="square,pad=0.3", facecolor="none", edgecolor="none", alpha=1.0
    )
...
for database in database_list:
    ...
    sm_count_df = pd.DataFrame(sm_count_arr, index=frameworks, columns=frameworks)

    # makes a single column of SM counts
    sm_column = (sm_count_arr[:, 0] / total_molecule_count) * 100
    sm_column_df = pd.DataFrame(sm_column, index=None, columns=None)

    # %new dataframe
    percent_new_arr = (new_count_arr / unique_count_arr) * 100
    percent_new_df = pd.DataFrame(percent_new_arr, index=frameworks, columns=frameworks)
    ...
    percent_common_df = pd.DataFrame(
        percent_common_arr, index=frameworks, columns=frameworks
    )
    ...
    percent_rel_common_df = pd.DataFrame(
        percent_rel_common_arr, index=frameworks, columns=frameworks
    )

    GenerateHeatmap(
        percent_rel_common_df,
    ...
    percent_common_df.to_csv(
        f"percent_common_dfs/percent_common_df_{database}.csv", index=True
    )
    sm_column_df.to_csv(f"percent_sms_dfs/sm_percentages_df_{database}.csv", index=True)
    percent_rel_common_df.to_csv(
        f"percent_rel_common_dfs/percent_rel_common_df_{database}.csv", index=True
    )
