...
def count_symmetric(frameworks, symmetric_filename):
        ...
        for idx2, framework2 in enumerate(frameworks):
            if framework1 != framework2:
                ...
                for line in symmetric_file_lines:
                    ...
                    if (
                        line_prefix[0] == line_prefix_match
                        and line_prefix[0] != f"pyrazine2{framework2}"
                    ):
                        ...
                        symmetric_metric = (
                            int(symmetric_molecules_number)
                            / int(total_molecules_number)
                        ) + 1
    ...
    # SMs are listed on left side column, products are listed across the top
    symmetric_count_df = pd.DataFrame(
        symmetric_count_arr, index=frameworks, columns=frameworks
    )
...
def GenerateHeatmap(dataframe, title, filename, sm_df):
    ...
    ax1.set_xlabel(
        "PDT Substructure", fontsize=14, fontweight="bold", fontfamily=fontfamily
    )
    ax1.set_ylabel(
        "SM Substructure", fontsize=14, fontweight="bold", fontfamily=fontfamily
    )
