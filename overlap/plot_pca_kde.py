...
def find_optimal_bandwidth(points):
    params = {'bandwidth': np.logspace(-1, 1, 20)}  # Search between 0.1 and 10
...
# Function to plot KDE contours on the right subplot with optimal bandwidth
def plot_kde_contours(ax, points1, points2, color1, color2, label1, label2, xlim, ylim):
    ...
    # Find optimal bandwidth for each dataset
    bandwidth1 = find_optimal_bandwidth(points1_scaled)
    print(f"Plotting KDE for {label1} with bandwidth {bandwidth1}")
    bandwidth2 = find_optimal_bandwidth(points2_scaled)
    print(f"Plotting KDE for {label2} with bandwidth {bandwidth2}")
    
    # Compute KDE for points1 with optimal bandwidth
    ...
    # Compute KDE for points2 with optimal bandwidth
...
# Function to compute KDE overlap with optimal bandwidth selection
def compute_kde_overlap(points1, points2, grid_size=100):
    # Standardize the data
    scaler = StandardScaler()
    points1 = scaler.fit_transform(points1)
    points2 = scaler.transform(points2)
    
    # Find optimal bandwidth for each set of points
    bandwidth1 = find_optimal_bandwidth(points1)
    print(f"Computing KDE for the first dataset with bandwidth: {bandwidth1}")
    bandwidth2 = find_optimal_bandwidth(points2)
    print(f"Computing KDE for the second dataset with bandwidth: {bandwidth2}")

    
    # Compute KDE with optimal bandwidth
    kde1 = KernelDensity(kernel='gaussian', bandwidth=bandwidth1).fit(points1)
    kde2 = KernelDensity(kernel='gaussian', bandwidth=bandwidth2).fit(points2)
...
def smiles_to_combined_kde(smiles_list1, smiles_list2, library1, library2):
    ...
    # Capitalize library names for plotting and add title
